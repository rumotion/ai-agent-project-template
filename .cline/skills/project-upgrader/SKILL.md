...
> 3. **Context Budget Enforcement**: Ensure target `AGENTS.md` stays <= 4,700 chars and `memory-bank/handoff.md` stays <= 1,200 chars to satisfy `FAST_INIT` budget checks.
...
2. Compact `AGENTS.md` to <= 4,700 chars if necessary.
...
### Phase 5: Validator, Benchmarks & Memory Bank

1. Copy latest `scripts/check-template.py` (v0.8.0 full validation suite).
2. Copy `scripts/benchmark-context.py` and `benchmarks/context/`.
3. Add missing `.gitignore` patterns (e.g. `.agent-benchmarks/`).
4. Update `memory-bank/` files (`00-index.md`, `startup.md`, `reminders.md`, `model-routing.md`, `handoff.md`) to schema v2 while keeping all project status history.
