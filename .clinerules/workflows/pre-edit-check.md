...
2. **Consult Memory Bank Risks & Decisions**: Search `memory-bank/risks.md` and `memory-bank/decisions.md` for past failure patterns, disproven hypotheses, or architectural rules affecting target paths.
...
5. **Formulate Falsifiable Prediction**: State the expected baseline failure/state before the edit and the concrete observable outcome/diff after the edit. If reality diverges upon test execution, treat it as a refutation signal, halt, and update notes before re-editing.
6. **Enforce Safety Boundaries**: STOP and seek explicit human approval if the edit involves:
