...
- Falsifiable Predictions & Pre-Execution Expectation: Formulate concrete baseline failure and expected passing diff before executing modifying edits. Misses trigger immediate refutation learning.
- 4-Tier Problem-Solving Escalation Ladder: Tier 1 (surgical fix) $\rightarrow$ Tier 2 (scratch script in `tmp/`) $\rightarrow$ Tier 3 (mock harness/simulator) $\rightarrow$ Tier 4 (architecture/state-machine redesign).
- Bimodal Execution: Single narrow probes during exploration; aggressive parallel batching during verified plan execution.
- Compaction-Resistant Refutation Ledger: Record dead hypotheses and killing evidence in `activeContext.md` / `risks.md` to avoid regression loops across model handoffs and compactions.
